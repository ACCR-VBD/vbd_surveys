...
#' 
#' Most participants (77%) reported that their authority is currently involved in the implementation of measures or activities related to diseases transmitted by ticks and mosquitoes, such as public relations, monitoring, or control (Table 1). Among the 12 respondents who indicated no current involvement, the most frequently cited reasons included a lack of financial or human resources (50%), a perception that such diseases are outside the authority's responsibilities (42%), and low perceived priority (8%). Regarding the existence of cantonal strategies or action plans, 48% of respondents were aware of a plan for mosquito-borne diseases and 9% for tick-borne diseases, while 33% reported that no strategy exists in their canton. Half of the respondents (50%) indicated that their authority lacks human resources to develop or implement relevant activities, and 56% stated they have no dedicated budget. Only a minority reported having specific resources for tick-borne (11%) or mosquito-borne (39%) diseases. 
#' 

shs_3 %>%  
  dplyr::select(starts_with("b_1"), -b_1_2,starts_with("b_2___"),,starts_with("b_3___"),,starts_with("b_4___"))  %>%
  gtsummary::tbl_summary(missing = "no")  %>%
  sh_100_add_header("> If you answered no, what are the reasons?",4) %>% 
  sh_100_add_header("> Do you know of any strategy or action plan in your canton regarding 
                    diseases transmitted by ticks and mosquitoes?",16) %>%
  sh_100_add_header("> Does your authority have human resources to develop and implement 
                    activities related to diseases transmitted by ticks and mosquitoes?",21) %>%
  sh_100_add_header("> Does your authority have a budget for the development and implementation 
                    of activities in the field of diseases transmitted by ticks and mosquitoes?",26) %>%
...
#' 
#' Reported concrete measures and activities in the field of vector-borne diseases varied markedly between tick- and mosquito-borne diseases (Table 2). The most common actions cited for ticks included public information campaigns (22%) and targeted awareness of professionals (13%), with rare reports of monitoring (4% passive; 0% active) and vaccination advice (6%). In contrast, involvement in mosquito-related activities was broader and more diverse: 41% of respondents reported engaging in public relations and professional awareness efforts, while active and passive mosquito monitoring were cited by 35% and 31% of respondents, respectively. Nearly one-third (30%) mentioned verifying sightings at new locations, and around 15–17% reported involvement in mosquito control on public or private land. Information and training activities were mentioned by 20% of respondents. Topics covered in awareness materials included vaccination (63%), protection against bites (50%), and elimination of breeding sites (44%). Open-text responses further highlighted involvement in national vaccination campaigns (e.g., against tick-borne encephalitis or bluetongue disease), interdepartmental coordination, media engagement, and surveillance of animal diseases transmitted by ticks and mosquitoes as defined in the Animal Disease Ordinance. Participants reported using a wide range of channels to disseminate information, most commonly official websites, emails, flyers, and direct communication with veterinarians or animal owners. Additional methods included newsletters, social media, articles in agricultural media, information sessions, and training courses, often in collaboration with municipalities or other local actors.
#' 

shs_3 %>%  
  dplyr::select(starts_with("c_1___"),starts_with("c_2___"),starts_with("c_3___"))  %>%
  gtsummary::tbl_summary(missing = "no")  %>%
  sh_100_add_header("> What activities is your authority currently 
                    (2024/2025) implementing in the field of tick-borne diseases?",0) %>% 
  sh_100_add_header("> What activities is your authority currently 
                    (2024/2025) implementing in the field of mosquito-borne diseases?",11) %>% 
  sh_100_add_header("> If your authority develops information, what subjects are covered?",23) %>% 
...
#' 
#' Respondents identified a range of knowledge gaps within their authorities concerning both tick- and mosquito-borne diseases (Table 3). For tick-borne diseases, the most commonly cited areas of limited knowledge included pathogen detection and disease management (both 19%), followed by prevention (17%). Open-text responses often emphasized limited relevance or jurisdictional responsibility, especially within animal health services, while some highlighted a lack of resources or the need for clearer coordination and federally supported monitoring. For mosquito-borne diseases, the main knowledge gaps were in disease and vector management and control (35%), as well as prevention and pathogen detection (20% each). Several respondents pointed to uncertainties around the appropriate use of adulticides, responsibilities among stakeholders, and long-term control strategies. Regarding actual control measures against mosquitoes, over half of participants (54%) reported no active mosquito control measures in place. Among those that did report activities, biological larvicides (24%) and elimination of breeding sites (19%) were the most common, while use of adulticides (11%) and non-chemical larvicides (7.4%) was less frequent. Open-text answers revealed a mix of pilot studies, public awareness campaigns, and experimental approaches like sterile insect techniques (SIT), often in collaboration with academic or federal partners. Some respondents indicated that control responsibilities lie with other agencies, such as municipalities or environmental offices. 
#' 

shs_3 %>%  
  dplyr::select(starts_with("c_6___"),starts_with("c_7___"),starts_with("c_8___"))  %>%
  gtsummary::tbl_summary(missing = "no")  %>%
  sh_100_add_header("> Among the following fields of action, where is the greatest need for information 
                    or the greatest lack of knowledge about tick-borne diseases in the authority you work for?",0) %>% 
  sh_100_add_header("> Among the following fields of action, where is the greatest need for information 
                    or the greatest lack of knowledge about mosquito-borne diseases in the authority you work for?",7) %>% 
  sh_100_add_header("> What measures is your authority implementing to control or combat invasive mosquitoes?",14) %>% 
...
#' 
#' Collaboration and coordination emerged as important themes in participants’ responses regarding vector-borne disease control (Table 4). While 61% of respondents reported active networks or exchanges with researchers and specialists for mosquito-borne diseases, only 20% indicated such collaborations for tick-borne diseases, and over a quarter (26%) reported no active exchange at all. Open-text responses highlighted needs for stronger inter-cantonal coordination, improved knowledge transfer, and harmonized federal guidelines, particularly for outbreak preparedness, surveillance, and the responsible use of control tools like biocides or adulticides. Several participants emphasized the importance of financial and human resource support, centralized information platforms, and a coordinated national monitoring strategy. Others cited challenges related to fragmented responsibilities across levels of government or lack of expertise and capacity. When asked about the integration of tick- and mosquito-borne disease issues into national and cantonal climate adaptation strategies, most respondents indicated limited awareness or engagement: 65% were unsure about national-level integration, and 52% did not know whether these issues were addressed in their own canton’s strategy. 
#' 

shs_3 %>%  
  dplyr::select(starts_with("d_1___"),starts_with("d_3___"),starts_with("d_4___"))  %>%
  gtsummary::tbl_summary(missing = "no")  %>%
  sh_100_add_header("> Does your authority maintain an active network or exchanges with 
                    researchers and specialists in the field?",0) %>% 
  sh_100_add_header("> Is the issue of tick- and mosquito-borne diseases sufficiently or inadequately addressed in the implementation of the national climate change adaptation strategy?",5) %>% 
  sh_100_add_header("> Is the issue of diseases transmitted by ticks and mosquitoes sufficiently or insufficiently taken into account in the implementation of your canton's climate change adaptation strategy (if any)?",12) %>% 
...
#' 
#' Collaborative networks are more commonly established around mosquito-related issues, with 61% of respondents indicating active exchanges with researchers and specialists in that domain, compared to only 20% for tick-related issues. Many participants called for stronger coordination across cantons, improved knowledge transfer, and clearer federal leadership, especially regarding outbreak preparedness, standardized intervention strategies, and resource sharing.
#' 
